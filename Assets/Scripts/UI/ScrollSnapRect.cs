...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    public bool isSetStartPageEnable;
    ...
    void Start()
    {
        ...
        int k = 0;
        foreach (Transform t in _container) if (t.gameObject.activeSelf) k++;
        _pageCount = k;
    ...
    private void SetPagePositions()
    {
        ...
        for (int i = 0; i < _pageCount; i++)
        {
            ...
            if (_horizontal)
            {
                childPosition = new Vector2(i * (width + _container.GetComponent<HorizontalLayoutGroup>().spacing), 0f);
            }
            else
            {
                childPosition = new Vector2(0f, -i * (height + _container.GetComponent<VerticalLayoutGroup>().spacing));
            }
    ...
    {
        //setButtomPage(aPageIndex);
        SetPageSelection(aPageIndex);
    ...
    //------------------------------------------------------------------------
    private void LerpToPage(int aPageIndex, bool isDelay)
    {
        setButtomPage(aPageIndex);
        aPageIndex = Mathf.Clamp(aPageIndex, 0, _pageCount - 1);
    ...
    public bool isSetButtonPage = false;
    public void setButtomPage(int aPageIndex)
    {
        if (isSetButtonPage)
        {
            if (aPageIndex == 0)
    ...
    {
        LerpToPage(_currentPage + 1, true);
    }
    ...
    public void OnEndDrag(PointerEventData aEventData)
    {
        ...
        else
        {
            ...
            LerpToPage(GetNearestPage(), false);
        }
